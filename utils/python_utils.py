...
class SingalPythonExecutor:
    ...
    @classmethod
    def worker(cls, command, globals):
        try:
            local_namespace = {}
            exec(command, globals, local_namespace)
            result = local_namespace.get("result")
    ...
    def run(self, command, timeout=3):
        ...
        try:
            result = self.worker(command, self.globals)
        except TimeoutError:
            result = None
        finally:
...
def execute_function(function_string, inputs, timeout=3, verbose=False):
